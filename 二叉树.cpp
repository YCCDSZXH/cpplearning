...
class BinTree  //二叉树类
{
 private:
  ...
  {
    queue<BinNode<string>*> q;
    if (tmp) q.push(tmp);
    while (!q.empty()) {
      BinNode<string>* temp = q.front();
      visit(temp);
      q.pop();
  ...
  {
    if (tmp)
      return max(BinTreeDepth(tmp->left()), BinTreeDepth(tmp->left())) + 1;
    else
      return 0;
  }
  ...
  {
    if (tmp)
      return BinTreeNodes(tmp->left()) + 1 + BinTreeNodes(tmp->right());
    else
      return 0;
  }
  int BinTreeHeight(BinNode<E>* tmp)  //获得二叉树的高度
  {
    return BinTreeDepth() + 1;
  }
